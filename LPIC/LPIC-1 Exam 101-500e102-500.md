...
- `ps -u` (pode usar também `ps -u usuario`): modo de exibição de usuário (mostra mais opções), sendo as colunas:
    ...
    - **RSS**: Memória residente usada pelo processo (em kilobytes)
    - **TTY**: `Terminal Type`: O terminal associado ao processo. Um terminal físico ou virtual (como `pts/0`, que representa uma conexão remota via `SSH`). Se o processo não estiver associado a nenhum terminal, pode aparecer como `?`
    - **STAT**: estado do processo (por exemplo, `R` para running, `S` para sleeping, `Z` para zombie, etc.)
    - **START**: horário que processo foi iniciado
