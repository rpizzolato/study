...
-`ifconfig enp0s8 down`: baixa a interface (se der `ifconfig`, não mostratá mais, para mostrar precisar usar `ifconfig -a` - display all interfaces which are currently available, even if `down`)
...
- `hostname -f` (ou `--fqdn` ou `--long`): mostra o full qualified domain name (nome completo - hostname + domínio). Nome de domínio pega do `/etc/hosts`. Pega informações do NIS domain name.
**NIS (Network Information Service)**, também conhecido como **YP (Yellow Pages)**, é um serviço de diretório usado em sistemas Unix e Linux para compartilhar informações administrativas de forma centralizada em uma rede. Ele é projetado para distribuir dados como usuários, senhas, grupos, informações de hosts, entre outros, para facilitar a administração de sistemas em redes locais.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNzA5NDQyMDUsNDA3MjY3NzM5LC0xMD
Q3MTQyODIyLDEyOTQ2NjkyODUsMzY5ODY1MTk1LDEyMTYzMzYz
NTksMjE2NDI1NTkxLDE1MTAzNTU5MTUsLTE4NDQ1MTI5MzcsLT
M5NTI5NjI2MSwtMTcwMzk0MjQyOSwxMDY1ODk3OTI3LC00NTk4
OTY3MjcsLTI3OTYwOTcxNiw1MTk3NDkwNjksNTQ0NzY3NzgzLD
MxNjkzMjExMSwtMTE3NjkyNTkwOSwtNjc5NTAyNzQ1LC0xNjk5
NzMxOTkyXX0=
-->