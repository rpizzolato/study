...
Em `systemctl status rsyslog`, podemos obter o arquivo que carrega o **rsyslog**, no caso `rsyslog.service`. E se executar um `cat` nele (`cat /lib/systemd/system/rsyslog.service`) é possível ver ele requer (no campo **Requires**) o socket `syslog.socket`.
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk1Njc4MjMzMiwtMTA4ODY0MDA3NSwxMT
