...
- `/etc/resolv.conf`: determinar qual servidor DNS será utilizado pelo sistema. Indica o `nameserver`, que é basicamente o servidor DNS que irá usar. (mais detalhes no tópico **109.3**)
...
Supondo que a saída do comando `nmcli connection` seja: 
...
>[!NOTE]
>
>`nmcli device wifi connect`, apenas o SSID é obrigatório, a senha pode ser omitida caso a conexão já tenha sido criada anteriormente
...
Primeiramente precisa verificar se o pacote `ifupdown` está instalado: `dpkg -l | grep ifupdown`, caso não tenha, instalar: `apt install ifupdown -y` (nas distros mais recentes esse pacote foi retirado devido a entenderem que o que deve ser utiliado).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NTk2ODQ3OTgsLTI1OTY0NjU2MywtMT
YyOTA2NTE5MSwtMTAxMjQ3MTEsNDgwMTkwNjUzLC0xOTczOTYx
NTQyLC0yMTA0MjI2MzgsODM2MjY4MzAyLDE4NDczMDEwNzYsMj
AwMTg2ODQyMCwtNDM1NzE0NjIyLDE1MzUzMDI0MzMsLTEyNDUy
MTE2ODMsNzk0OTA4NjEwLC0xNTIwODAwNTksLTUxOTI4NjA2OS
wtMjA5NzAyNjgwMywtODg1OTk3MDcwLC04MjM1NjE4MiwxNDU1
MjAwODQ0XX0=
-->