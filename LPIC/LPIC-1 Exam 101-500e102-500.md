...
- `lastb`: histórico de usuários que tentaram se logar, mas erraram a senha, ou usuário não existia. Esses dados (tanto do `last`, como `lastb`) são consultados de `/var/log/wtmp` (arquivo binário). Para ler usar last -f /var/log/wtmp (-f ou)

## 110.2 Configurar a segurança do host

## 110.3 Proteção de dados com criptografia
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA2OTcyMzU3MSwtMTk1ODU4OTE3NSwtMT
