...
O primeiro caracter, no caso acima, `-`, que fica entre a segunda coluna (que indica o número de links físicos que apontam para aquele arquivo - no caso o número `2`) entre `lpi1` e `-rw-rw-r--`, significa:
...
Sequencia: permissões do user, group e others

sendo: lpi1 o dono do arquivo file e suporte é o grupo

Portanto, o usuário dono lpi1 tem permissão de leitura e escrita (rw) e os outros usuário (grupo suporte), também tem permissão de leitura e escrita. Os outros somente leitura.
...
- = (definir com um valor específico)
...
*Não está sequencial, olhar sempre para o ls lá no começo, como referência para os comandos acima listados.

*O igual (=) deixa realmente igual a, removendo outros atributos que tenha. Ex. Se deixarmos rw, caso tenha rwx, vai deixar apenas rw (removendo o x).

chmod 0=w file.txt => -rw-rw--w-

chmod 0=rw file.txt => -rw-rw-rw-

chmod 0=x file.txt => -rw-rw---x

**Esse modo é conhecido como modo simbólico (o outro é o octal)

chmod ug+rw-x,o-rwx text.txt: é possível combinar separando por vírgula

chmod 660 text.txt: equivalente em octal
