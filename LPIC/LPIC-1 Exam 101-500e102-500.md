...
ldd /usr/bin/vi: imprime todas as dependências (bibliotecas compartilhadas, geralmente tudo que tem o .so) que o vi utiliza, imprime também o endereço de memória hexadecimal que ele é carregado
...
*Uso com -v traz mais informações

*O programa que faz esse vínculo é o ld.so ( ou ld-linux.so), que busca em 2 diretórios: /lib e /usr/lib

*No entanto é possível configurar o ld.so para que encontre lib em outros locais.
...
Local que é mapeado as bibliotecas no linux

Em /etc/ld.so.cache ficam mapeados todas as bibliotecas utilizadas e conhecidas pelo linux que podem ser utilizadas pelas aplicações. 

Podemos listar o conteúdo: ls -ld /etc/ld.so*, que vai aparecer os arquivos ld.so.cache, ld.so.conf e o diretório ld.so.conf.d.

ld.so.conf é o principal arquivo de configuração, é onde podemos indicar novos caminhos para as bibliotecas.

Para incluir uma nova biblioteca, podemos realizar no ld.so.conf ou criar um arquivo .conf dentro de ld.so.conf.d

*O ld.so.conf tem um include que incluir tudo que for *.conf dentro de ld.so.conf.d

*Esses arquivos *.conf devem incluir caminhos absolutos para os diretórios da biblioteca compartilhada
...
ldconfig: lê as mudanças em ld.so.conf e dos arquivos dentro de ld.so.conf.d e recompila (regera) o arquivo ld.so.cache

ldconfig -p (--print-cache): lista todas as bibliotecas ativas no momento
...
LD_LIBRARY_PATH está para as bibliotecas compartilhadas como PATH está para os executáveis.
...
objdump: display information from object files (confira se ele está instalado em seu sistema com which objdump)

Use objdump com -p (ou --private-headers) e grep para:
