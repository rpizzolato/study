...
- `/etc/rsyslog.conf`: arquivo principal de configuração do rsyslog. A primeira vista temos os módulos (`module(load="imuxsock")`), que nesse caso serve para comunicação do `rsyslog` com `systemd-journal`, eles se comunicam.



<!--stackedit_data:
eyJoaXN0b3J5IjpbODA1MjMyMzkyLC01MjAwNTYyMzddfQ==
-->