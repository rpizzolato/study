...
Em sistemas com o GRUB instalado, eles costumam estar localizados em /boot/grub e incluem o arquivo de configuração:

- (/boot/grub/grub.cfg para o GRUB 2 ou 

- /boot/grub/menu.lst no caso do GRUB Legacy)

- módulos (em /boot/grub/i386-pc)

- arquivos de tradução (em /boot/grub/locale) e 

- fontes (em /boot/grub/fonts) do GRUB.




O 1º disco de um sistema é geralmente o dispositivo de inicialização . Pode ser necessário saber se existe uma partição de inicialização no disco. Para isso existe o utilitário fdisk. Listar todas as partições no 1º disco da sua máquina:
```
# fdisk -l /dev/sda
```
A partição de inicialização é identificada com o * na coluna boot.

Crie um diretório temporário em /mnt e monte a partição nele:
...
Caso esteja instalando em um sistema que não possui uma partição de inicialização, mas apenas um diretório /boot no sistema de arquivos raiz, aponte-o em grub-install. Nesse caso, o comando é: # grub-install --boot-directory=/boot /dev/sda


#### alterando parâmetro no boot

No bootloader, podemos apertar o botão `e` para editar o boot, com isso na linha linux:
```
